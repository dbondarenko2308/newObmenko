...
$(document).ready(function() {
	...
	$('.showcase__sel--head').on('click', function() {
		$(this).parent().toggleClass('is-selected')
	})

	$(function() {
		var $select = $('.showcase__sel')

		$select.find('[data-select-name]').on('click', function() {
			var text = $(this).text().trim()

			$select.find('.showcase__sel--name span').text(text)

			$select.addClass('is-selected')
			$('.showcase__sel').removeClass('is-selected')
		})
	})

	$(function() {
		function animateLogos(selector) {
			var $logos = $(selector)
			var index = 0
			var duration = 600
			var delay = 4000
			...
			function nextLogo() {
				...
				$({ scale: 1 }).animate(
					{ scale: 0 },
					{
						duration: duration,
						step: function(now) {
							$current.css('transform', 'scale(' + now + ')')
						},
						complete: function() {
							index = (index + 1) % $logos.length
							var $next = $logos.eq(index)
							...
							$({ scale: 0 }).animate(
								{ scale: 1 },
								{
									duration: duration,
									step: function(now) {
										$next.css('transform', 'scale(' + now + ')')
									},
									complete: function() {
										setTimeout(nextLogo, delay)
									}
								}
							)
						}
		...
		animateLogos('.logo__svg')

		animateLogos('.logo__svg--white')
	})
})
